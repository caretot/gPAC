...
const url = `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/gemeta?key=${apiKey}`;
...
function populateTable(rows) {
    ...
    rows.slice(1).forEach((row, index) => {
      ...
      const keysToColor = [4, 5, 7, 8, 10, 11, 13, 14];
      ...
      expandableRow.innerHTML = `
        <td colspan="4">
          <div class="detail-item">
            ${createSpan('L0s1', row[4], maxValue, minValue, mediumValue)}
            ${createSpan('L0s2', row[5], maxValue, minValue, mediumValue)}
            <span><strong>L0:</strong> ${row[6] || 'N/A'}</span>
          </div>
          <div class="detail-item">
            ${createSpan('L1s1', row[7], maxValue, minValue, mediumValue)}
            ${createSpan('L1s2', row[8], maxValue, minValue, mediumValue)}
            <span><strong>L1:</strong> ${row[9] || 'N/A'}</span>
          </div>
          <div class="detail-item">
            ${createSpan('L2s1', row[10], maxValue, minValue, mediumValue)}
            ${createSpan('L2s2', row[11], maxValue, minValue, mediumValue)}
            <span><strong>L2:</strong> ${row[12] || 'N/A'}</span>
          </div>
          <div class="detail-item">
            ${createSpan('L3s1', row[13], maxValue, minValue, mediumValue)}
            ${createSpan('L3s2', row[14], maxValue, minValue, mediumValue)}
            <span><strong>L3:</strong> ${row[15] || 'N/A'}</span>
          </div>
        </td>
      `;
