...
async function fetchStudentData() {
  ...
  try {
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/CE!AE6:AW45?key=${API_KEY}`;
...
studentRow.slice(8, 12).forEach((cell) => {
    chartYValues.push(Number(cell)); // Ensure the values are numeric
});

// Chart configuration
const xValues = ["CHEM ENG", "CHEMISTRY", "PROFESSIONAL PREPARATION", "INTERNSHIP", "MISSED"];
const barColors = ["#4174C9", "#3C9D4E", "#7031AC", "#C94D6D", "#000000"]; // Black color for MISSED

const RANGE = "CE!AM4:AW4";  // The range to fetch data from
