...
// Chart configuration
const xValues = ["Math", "PHYCE", "CHENG", "LANG", "ACAD", "MISSED"];
const barColors = ["#b91d47", "#00aba9", "#2b5797", "#e8c3b9", "#1e7145", "#000000"]; // Black color for MISSED

const RANGE = "CE!AU4:BK4";  // The range to fetch data from
...
async function createChart() {
  ...
  const trace = {
    labels: sortedLabels,  // Sorted labels
    values: sortedValues,  // Sorted values
    type: 'pie',
    textinfo: 'percent',  // Display only the percentage on the pie slices
    textposition: 'inside',  // Position the text inside the slices
    marker: {
      colors: barColors.slice(0, sortedLabels.length),  // Use the same colors for the sorted labels
    }
  };
